...
def add_user():
	...
	try:		
		...
		if _name_equipment and _type_equipment and _make_country and _price and _purpose and  _trainer  and _class_name    and  _room  and  _time_of_class and request.method == 'POST':
			sql = "INSERT INTO equipment_table (name_equipment, type_equipment, make_country, price, purpose , trainer_name ,class_name , room ,time_of_class ) VALUES(%s, %s, %s, %s, %s,%s, %s, %s, %s )" #same tables
			data = (_name_equipment, _type_equipment, _make_country, _price, _purpose, _trainer , _class_name ,_room  , _time_of_class) #same tables 
			conn = mysql.connect()
			cursor = conn.cursor()
...
def update_user():
	...
	try:		
		...
		if _name_equipment and _type_equipment and _make_country and _price and _purpose and  _trainer  and _class_name    and  _room  and  _time_of_class and  request.method == 'POST':
			...
			data = (_name_equipment, _type_equipment, _make_country, _price, _purpose, _trainer , _class_name ,_room  , _time_of_class ,  _id) #same tables
			conn = mysql.connect()
			cursor = conn.cursor()
			cursor.execute(sql, data)
			conn.commit()
